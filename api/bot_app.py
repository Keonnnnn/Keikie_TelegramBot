"""
Keke Bill Splitter Bot — improved edition
Features:
  - Equal and Individual split modes
  - Named items per person
  - Shared items split among a subset of people
  - /undo to step back one prompt at any time
  - /restart to restart the whole flow
  - Tip, GST, service charge support
  - Per-person item breakdown in the final summary
  - Inline keyboards for binary choices
...
# ─────────────────────────────────────────────────────────
(
    CHOICE,           # 0  equal / individual
    TOTAL,            # 1  equal: total bill
    PEOPLE_EQUAL,     # 2  equal: number of people
    PEOPLE_INDIV,     # 3  individual: number of people
    NAME_INDIV,       # 4  individual: collect person name
    ITEM_COUNT,       # 5  individual: how many items for current person
    ITEM_NAME,        # 6  individual: item name
    ITEM_AMOUNT,      # 7  individual: item amount
    REVIEW_PERSON,    # 8  individual: confirm / edit person's items
    SHARED_CONFIRM,   # 9  ask whether shared items exist
    SHARED_NAME_AMT,  # 10 shared item name + amount
    SHARED_PEOPLE,    # 11 which people share that item
    GST,              # 12
    SERVICE,          # 13
    TIP,              # 14
) = range(15)
...
def push_history(context: ContextTypes.DEFAULT_TYPE, state: int) -> None:
    """Save a snapshot of user_data + the current state before mutating."""
...
def _re_prompt(state: int, data: dict) -> str:
    prompts = {
        CHOICE:         "How do you want to split the bill? Reply *Equal* or *Individual*.",
        ...
        REVIEW_PERSON:  "Review or confirm the items above. Reply *done* to continue or *edit N* to remove item N.",
...
async def undo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keys_to_remove = [k for k in context.user_data if k != "_history"]
...
def build_summary(data: dict) -> str:
    ...
    if split_type == "equal":
        ...
        person_base: dict = {}
        for name in names:
            person_base[name] = sum(a for _, a in amounts_by.get(name, []))
        ...
        for name in names:
            ...
            for iname, iamt, sharers in shared_items:
                if name in sharers:
                    lines.append(f"      • {iname} (÷{len(sharers)}): {fmt(iamt / len(sharers))}")
    ...
    lines += ["", "━━━━━━━━━━━━━━━━━━━━━", "_Generated by @Keikie\\_Bot_"]
    return "\n".join(lines)
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "*Individual split* — enter each person's name, their items (with names "
        "and prices), optional shared items, then GST / service charge / tip.\n\n"
        "At any prompt:\n"
...
async def get_total(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("⚠️ Please enter a valid positive number (e.g. `45.80`).", parse_mode="Markdown")
        return TOTAL
...
async def get_people_equal(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(build_summary(context.user_data), parse_mode="Markdown")
...
async def get_individual_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("⚠️ Please enter a valid amount (e.g. `13.95`).", parse_mode="Markdown")
        return ITEM_AMOUNT
...
async def _show_person_review(update: Update, context: ContextTypes.DEFAULT_TYPE, name: str) -> int:
    ...
    lines += ["", "Reply *done* to confirm, or *edit N* to remove item N (e.g. `edit 2`)."]
...
async def review_person(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text.startswith("edit "):
        ...
        if len(parts) == 2 and parts[1].isdigit():
            ...
            if 0 <= idx < len(items):
                ...
                await update.message.reply_text(
                    f"🗑 Removed *{removed[0]}* ({fmt(removed[1])}).",
                    parse_mode="Markdown",
                )
                if items:
                    return await _show_person_review(update, context, name)
    ...
    await update.message.reply_text(
        "⚠️ Reply *done* to confirm, or *edit N* to remove an item (e.g. `edit 2`).",
        parse_mode="Markdown",
    )
    return REVIEW_PERSON
...
async def _advance_to_next_person_or_shared(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "_(e.g. a bottle of wine split among specific people)_",
        parse_mode="Markdown",
...
async def shared_people(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not valid:
        await update.message.reply_text(
            f"⚠️ No valid names found. Known people: {', '.join(known)}\nPlease try again."
        )
        return SHARED_PEOPLE
...
async def get_gst(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("⚠️ Enter a valid percentage like `9` or `0`.", parse_mode="Markdown")
        return GST
    push_history(context, GST)
    context.user_data["gst"] = v
    await update.message.reply_text("Enter *service charge* percentage (e.g. `10`), or `0`.", parse_mode="Markdown")
    return SERVICE
...
async def get_service(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("⚠️ Enter a valid percentage like `10` or `0`.", parse_mode="Markdown")
        return SERVICE
    push_history(context, SERVICE)
    context.user_data["service"] = v
    await update.message.reply_text("Enter *tip* percentage (e.g. `15`), or `0`.", parse_mode="Markdown")
    return TIP
...
async def get_tip(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("⚠️ Enter a valid percentage like `15` or `0`.", parse_mode="Markdown")
        return TIP
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data.clear()
    await update.message.reply_text(
        "❌ Cancelled. Send /split to start a new split.",
        reply_markup=ReplyKeyboardRemove(),
    )
    return ConversationHandler.END
