...
def build_summary(context_data: dict) -> str:
    ...
    if split_type == "equal":
        ...
        amounts_by_person = context_data["amounts_by_person"]  # dict: name -> list of amounts
    ...
    lines += ["", "─────────────────────"]
    lines.append("Generated by @Keikie_Bot")
    return "\n".join(lines)
...
async def get_service_individual(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(summary, parse_mode="Markdown")
    await update.message.reply_text(
        "📋 You can copy the summary above and share it with your group!"
    )
    return ConversationHandler.END
