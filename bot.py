...
async def choose_split_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Please tap *⚖️ Equal* or *🧮 Individual*.", parse_mode="Markdown")
    return CHOICE
    ...
    if text.startswith("edit "):
        ...
        if len(parts) == 2 and parts[1].isdigit():
            ...
            if 0 <= idx < len(items):
                ...
                await update.message.reply_text(
                    f"🗑 Removed *{removed[0]}* ({fmt(removed[1])}).", parse_mode="Markdown")
                if items:
                    return await _show_person_review(update, context, name)
                await update.message.reply_text(
...
async def shared_name_amt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text(
            "⚠️ Use the format: `Name, amount` (e.g. `Wine, 45.00`)", parse_mode="Markdown")
        return SHARED_NAME_AMT
